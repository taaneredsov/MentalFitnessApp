...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const { userId, email, password } = setPasswordSchema.parse(req.body)
    ...
    // Security: only allow setting password if there's no existing hash
    if (existingHash) {
    ...
    const updatedRecord = await base(tables.users).find(userId)
